...
if (isset($_POST['application-submit'])) {
    if (!empty($_POST)) {
        global $wpdb;
        $ULRN = randomString(10);
        $time = date('Y-m-d H:i:s');
        $wpdb->query($wpdb->prepare('INSERT INTO ' . WONDALOANS_TABLE_NAME . '(`ulrn`, `apr`, `status`,`created`) VALUES (%s,%s,%s,%s)', $ULRN, get_option('wondaloan_rate_name'),'Processing', $time));
        //Get last inserted id
        $data_id = $wpdb->insert_id;

        foreach ($_POST as $key => $value) {
            if ($key != 'application-submit') {
                $wpdb->query($wpdb->prepare('INSERT INTO ' . WONDALOANS_DATA_ENTRY_TABLE_NAME . '(`ulrn_no`, `data_id`, `fname`, `value`) VALUES (%s,%d,%s,%s)', $ULRN, $data_id, sanitize_text_field($key), sanitize_text_field($value)));
            }
        }
